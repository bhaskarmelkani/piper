...
| `built-in-tool-renderer.ts` | Custom compact rendering for built-in tools (for example `read`, `bash`, `edit`, `write`, `search_code`, `symbols_overview`) while keeping original behavior |
